...
export const schedulePublicationValidationResponse = object({
  ...
  violations: arrayOf(object({
    ...
    ruleVersion: nullable(integer),
    ruleName: nullable(text),
    expected: nullable(integer),
    actual: nullable(integer),
    time: nullable(text),
    affectedEmployeeIds: arrayOf(text),
    affectedShiftIds: arrayOf(text),
  })),
  coverage: arrayOf(object({
    date: { type: 'string', format: 'date' },
    time: text,
    count: integer,
    employeeIds: arrayOf(text),
    shiftIds: arrayOf(text),
    minRequired: nullable(integer),
    maxAllowed: nullable(integer),
    status: { type: 'string', enum: ['below', 'within', 'above'] },
  })),
});
