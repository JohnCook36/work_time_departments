...
Завершённая запись может быть исправлена только пользователем с
`ATTENDANCE_CORRECT` в данном отделе; первоначальные и новые timestamps
остаются в неизменяемых событиях и AuditLog. `SCHEDULE_READ` разрешает
просмотр только своего управляемого отдела, сотрудник видит лишь свои записи.
