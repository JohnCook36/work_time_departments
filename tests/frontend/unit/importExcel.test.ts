...
import { parseScheduleExcel } from '../../../src/services/excel/importExcel';
...
describe('parseScheduleExcel', () => {
  ...
  it('normalizes employee names and skips department and total rows', async () => {
    const file = await workbookFile([
      ['Отчёт за месяц'],
      ['Сотрудник', 'Пн 1'],
      ['Отдел Front Office'],
    ...
    const preview = await parseScheduleExcel(file, employees);
    expect(preview.matchedEmployees).toEqual(['Анна']);
    expect(preview.unknownEmployees).toEqual([]);
    expect(preview.entries).toEqual([
      { employeeName: 'АННА', employeeId: 'anna', day: 1, value: '08:00-17:00' },
    ]);
  });

  it('warns about duplicate day columns while preserving both parsed cells', async () => {
    const file = await workbookFile([
      ['Сотрудник', '1', 'День 1'],
      ['Анна', '08:00-17:00', '15:00-23:00'],
    ]);
    const preview = await parseScheduleExcel(file, employees);
    expect(preview.detectedDays).toEqual([1]);
    expect(preview.entries).toHaveLength(2);
    expect(preview.entries.map((entry) => entry.day)).toEqual([1, 1]);
    expect(preview.warnings.join(' ')).toMatch(/повторяются дни: 1/);
  });

  it('uses a valid actual-time row when the schedule cell is empty', async () => {
    const file = await workbookFile([
      ['Сотрудник', '1'],
      ['Анна', ''],
      ['Факт', '15:00-23:00'],
    ...
    const preview = await parseScheduleExcel(file, employees);
    expect(preview.entries).toEqual([
      { employeeName: 'Анна', employeeId: 'anna', day: 1, value: '15:00-23:00' },
    ]);
  });
